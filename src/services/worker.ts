...
async function processJob(job: any) {
  ...
  try {
    ...
    if (job.type === 'youtube' && job.url) {
      ...
      const ytCmd = `"${ytDlpPath}" --ffmpeg-location "${safeFfmpegPath}" --remote-components ejs:github --js-runtimes node --no-check-certificates${cookiesArg} -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/best" --merge-output-format mp4 -o "${originalFilePath}" "${job.url}"`;
